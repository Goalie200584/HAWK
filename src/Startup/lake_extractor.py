...
def csv_write_lake_coords(filedir:str, outputdir:str):
    ...
    lake_file_name = filedir.split("/")[-1]
    lake_name = lake_file_name.split(".")[0]
    number_of_indents = 0
    ...
    with open(filedir) as file:
        ...
        for x in coord_lines:
            ...
            if csv_writes == 0:
                with open(outputdir + lake_name + "_Coords.csv", "x+") as file2:
                ...
                with open(outputdir + lake_name + "_Coords_is" + str(csv_writes) + ".csv", "x+") as file2:
                    file2.write("Longitude,Latitude,Altitude\n")
                    for i in coordinates:
                        if i == coordinates[0]:
                            file2.write(i[6 + number_of_indents:] + "\n")
